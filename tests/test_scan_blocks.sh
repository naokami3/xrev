...
screen=$'noise\n'"$B"$'\n開始マーカー: 説明文\n'"$E"$'\nmore'
assert_eq "エコー(JSON無し)は 0 件" "0" "$(printf '%s' "$screen" | _scan_review_blocks | head -1)"

# 2) テンプレート（不正 JSON）→ 0 件
screen="$B"$'\n{ ここに verdict と findings[] を持つ JSON }\n'"$E"
assert_eq "テンプレート(不正JSON)は 0 件" "0" "$(printf '%s' "$screen" | _scan_review_blocks | head -1)"

# 3) verdict を持たない妥当 JSON → 0 件（本物の応答ではない）
screen="$B"$'\n{"foo":1}\n'"$E"
assert_eq "verdict 無しの JSON は 0 件" "0" "$(printf '%s' "$screen" | _scan_review_blocks | head -1)"

# 4) 妥当な応答 1 件 → 1 件、中身（正規化JSON）を返す
screen=$'x\n'"$B"$'\n{"verdict":"approve","findings":[]}\n'"$E"$'\ny'
out="$(printf '%s' "$screen" | _scan_review_blocks)"
...
# 6) 何も無い画面 → 0 件
assert_eq "空画面は 0 件" "0" "$(printf '%s' "ただのログ出力" | _scan_review_blocks | head -1)"

# 7) TUI 折り返し + ガター字下げ + 文字列途中の改行（実 Codex 画面の再現）→ de-wrap して検出
wrapped="$B"$'\n  {\n  "verdict": "request_changes",\n  "findings": [\n  {\n  "file": "design",\n  "severity": "high",\n  "category": "bug",\n  "message": "これは長いメッセージで途中で折り返\n  されてガター字下げが付いています"\n  }\n  ]\n  }\n'"$E"
out="$(printf '%s' "$wrapped" | _scan_review_blocks)"
...
assert_eq "有効ブロック 3 件を数える" "3" "$(printf '%s' "$multi" | _scan_review_blocks | head -1)"

# 11) round_id フィルタ: 期待 round_id 指定時は一致ブロックのみ採用
rid_match="$B"$'\n{"round_id":"rX","verdict":"approve","findings":[]}\n'"$E"
rid_other="$B"$'\n{"round_id":"rY","verdict":"approve","findings":[]}\n'"$E"
assert_eq "round_id 一致は採用(1件)" "1" "$(printf '%s' "$rid_match" | _scan_review_blocks "rX" | head -1)"
assert_eq "round_id 不一致は不採用(0件)" "0" "$(printf '%s' "$rid_other" | _scan_review_blocks "rX" | head -1)"
assert_eq "round_id 欠落ブロックは期待指定時に不採用(0件)" "0" \
  "$(printf '%s' "$B"$'\n{"verdict":"approve","findings":[]}\n'"$E" | _scan_review_blocks "rX" | head -1)"
