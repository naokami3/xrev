...
**reviewer は「実ターミナル内の codex CLI」であること**（cmux のエージェント統合パネルは宛先解決の契約外）。

**reviewer の用意は create-if-missing（自動生成）が既定**: 一拍確認の中で reviewer の有無を確認し、無ければ
（`reviewer_autocreate` の方針に従って）自動生成する。ユーザーにスクリプトのパスを手実行させない:

```bash
# 1) まず有無を確認（同一WSに使える reviewer があるか）
"${CLAUDE_PLUGIN_ROOT}/scripts/transport.sh" resolve --json
# 2) 無い(ok:false) かつ reviewer_autocreate=ask なら、ユーザーに「reviewer を自動生成してよいか」を一度確認し、
#    了承を得たら生成する（ペイン生成はレイアウトを変える副作用なので無確認では作らない）:
"${CLAUDE_PLUGIN_ROOT}/scripts/transport.sh" ensure-reviewer   # 同一WSに無ければ codex ペインを1枚生成して採用
```

`ensure-reviewer` は冪等（あれば採用・無ければ生成）。既存が**壊れ/曖昧/別物**のときは作り直さず該当 exit
（14/16/17）で人間に委ねる。生成の競合は WS 鍵ロックで直列化し、競合期限切れは exit20。詳細・終了コードは
[references/protocol.md](../../references/protocol.md)。

手動で用意する場合は、reviewer 用に開いた実ターミナルで `scripts/start-reviewer.sh` を実行してもらってもよい
（自タブを `Review Codex` に設定して codex を exec する。インストール利用者はパスが分からないので通常は
ensure-reviewer の自動生成を使う）。
